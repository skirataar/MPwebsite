...
function SellerLoginContent() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/auth/register", {
        ...
        body: JSON.stringify({ name: suName, email: suEmail, password: suPassword, role: "SELLER" }),
      });
  ...
  return (
    <div className="w-screen min-h-screen bg-background text-on-surface antialiased flex flex-col justify-center items-center font-body-md px-md selection:bg-primary-container selection:text-on-primary-container relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-surface rounded-[24px] border-[0.5px] border-outline-variant p-lg flex flex-col gap-md shadow-lg mt-huge mb-md relative z-10">
        <div className="flex flex-col gap-xs text-center items-center">
          <div className="w-12 h-12 bg-primary rounded-xl flex items-center justify-center shadow-sm mb-xs">
            ...
            <form onSubmit={handleSignUp} className="flex flex-col gap-sm">
              <div className="flex flex-col gap-xs">
                ...
                <input
                  ...
                  placeholder="e.g. Jaipur Crafts Studio"
                  className={inputClass}
                />
              </div>
